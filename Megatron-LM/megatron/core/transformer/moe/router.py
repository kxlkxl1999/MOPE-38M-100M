...
from megatron.core.transformer.moe.OT_pruning import ot_based_ensemble_pruning, otep_batched
...
class TopKRouter(Router):
    ...
    def __init__(self, config: TransformerConfig) -> None:
        ...
        self.moe_sigma = torch.eye(E, dtype=torch.float32) # ← 占位
    ...
    def sinkhorn_load_balancing(self, logits: torch.Tensor):
        ...
        if self.training:
            with torch.no_grad(): # 用于包裹需要禁用梯度计算的代码段
    ...
    def aux_loss_load_balancing(self, logits: torch.Tensor):
        ...
        定义一个辅助损失 switch_load_balancing_loss_func，度量专家使用的不均衡程度，
        然后在 apply_load_balancing_loss 中将这部分损失合并到模型训练中
        Args:
        ...
        """
        # 考虑 expert capacity（容量限制），即每个专家最多接收 capacity_factor × (num_tokens / num_experts) 个 token，避免个别专家负载过重
    ...
    def routing(self, logits: torch.Tensor):
        ...
        if self.routing_type == "sinkhorn": # default is aux_loss
            ...
            # ---------- 1) 先用标准 top-k Softmax with capacity ----------
            scores, routing_map, _ = topk_softmax_with_capacity(
                logits,
                self.topk,
                capacity_factor=self.config.moe_expert_capacity_factor,
            ...
            # sigma_np = self.moe_sigma.cpu().numpy()  # (E,E) 只拷一次
            #
            # T, E = scores.shape
            # device = scores.device
            # new_routing = torch.zeros_like(routing_map)  # Bool 0
            #
            # for t in range(T):  # 逐 token
